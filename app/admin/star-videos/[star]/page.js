...
export default function AdminStarVideosPage(){
  ...
  useEffect(()=>{ fetchVideos(); },[star, currentPage, searchTerm]);
  useEffect(()=>{ fetchSuggestedTags(); fetchSuggestedStars(); },[]);

  const fetchVideos=async()=>{
    try{ setLoading(true);
      const searchQuery = searchTerm? `${star} ${searchTerm}`: star;
      const res = await fetch(`${apiUrl}/getpostdata?search=${encodeURIComponent(searchQuery)}&page=${currentPage}&limit=${itemsPerPage}`);
      ...
      <div className="admin-dashboard"><AdminNavbar/>
        <div className="admin-content"><div className="container"><div className="row"><div className="col-12"><div className="admin-table-container">
          <div className="admin-table-header">
            <div>
              <button className="btn btn-outline-secondary me-3" onClick={()=>{ const fromPage=searchParams.get('fromPage')||'1'; router.push(`/admin/stars-list?page=${fromPage}`); }}>← Back to Stars</button>
              <h2>Videos for Star: "{decodeURIComponent(star)}" ({totalVideos})</h2>
            </div>
